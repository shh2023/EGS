...
from featureanddeta_propagation import FeatureAndDetaPropagation
...
def featureanddeta_propagation(edge_index,X, feature_mask, num_iterations,lambda_1,lambda_2):
    propagation_model = FeatureAndDetaPropagation(num_iterations=num_iterations)

    return propagation_model.propagate(x=X,edge_index=edge_index,mask=feature_mask,lambda_1=lambda_1,lambda_2=lambda_2)
    

def filling(filling_method, edge_index, X, feature_mask, num_iterations, lambda_1, lambda_2):
    ...
    elif filling_method == "featureanddeta_propagation":
        X_reconstructed = featureanddeta_propagation(edge_index, X, feature_mask, num_iterations, lambda_1, lambda_2)
    else:
