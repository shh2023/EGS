...
parser.add_argument(
    ...
    default="featureanddeta_propagation",
    choices=["random", "zero", "mean", "neighborhood_mean", "feature_propagation","featureanddeta_propagation"],
)
parser.add_argument(
    "--model",
    type=str,
...
parser.add_argument("--missing_rate", type=float, help="Rate of node features missing", default=0.05)
parser.add_argument("--patience", type=int, help="Patience for early stopping", default=10000)
parser.add_argument("--lr", type=float, help="Learning Rate", default=0.001)
parser.add_argument("--epochs", type=int, help="Max number of epochs", default=500)
parser.add_argument("--n_runs", type=int, help="Max number of runs", default=5)
parser.add_argument("--hidden_dim", type=int, help="Hidden dimension of model", default=1024)
parser.add_argument("--num_layers", type=int, help="Number of GNN layers", default=2)
parser.add_argument("--lambda_1", type=float, help="Number of lambda_1", default=0.1)
parser.add_argument("--lambda_2", type=float, help="Number of lambda_2", default=0.1)
parser.add_argument(
    "--num_iterations", type=int, help="Number of diffusion iterations for feature reconstruction", default=30
)
parser.add_argument("--lp_alpha", type=float, help="Alpha parameter of label propagation", default=0.9)
parser.add_argument("--dropout", type=float, help="Feature dropout", default=0.9)
...
parser.add_argument(
    "--homophily", type=float, help="Level of homophily for synthetic datasets", default=True,
)
...
def run(args):
    ...
    data = dataset.data
    #pd_x = pd.DataFrame(data.x.cpu().numpy())
    #pd_y = pd.DataFrame(data.y.cpu().numpy())
    #pd_x.to_csv("x1.csv",header=None,index=None,startrow=111)
    #pd_y.to_csv("y1.csv",header=None,index=None,startrow=111)
    ...
    for seed in tqdm(seeds[: args.n_runs]):
        ...
        if args.model == "lp":
            ...
            start = time.time()
            filled_features = (
                filling(args.filling_method, data.edge_index, x, missing_feature_mask, args.num_iterations,args.lambda_1, args.lambda_2)
            ...
            for epoch in range(0, args.epochs):
                ...
                #     pd_x.to_csv("photo_pagnnx.csv",header=None,index=None)
                #     pd_y.to_csv("photo_pagnny.csv",header=None,index=None)
            (_, val_acc, test_acc), _ = test(model, x=x, data=data, logits=y_soft, evaluator=evaluator) #在所有epoch结束后，再次进行一次测试，得到最终的验证精度和测试精度。
